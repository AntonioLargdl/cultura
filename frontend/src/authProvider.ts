...
export const authProvider: AuthBindings = {
  forgotPassword: async ({ email }) => {    
    try {
      const response = await axios.post('https://culltura.onrender.com/usuarios/forgot-password', { email });
  ...
  updatePassword: async ({ password, id, token }) => {
    try{
      const response = await axios.post('https://culltura.onrender.com/usuarios/update-password', { password, id, token });
      if(response) {
        notification.success({
  ...
  login: async ({ access, password }) => {
    try {
      ...
      const response = await axios.post('https://culltura.onrender.com/usuarios/login', { access, password });
